...
Все 20 функций собраны в едином файле `algorithms.py` по предоставленному
...
**Акцент варианта 19** — бизнес-задача «Анализ больших данных (User IDs)»:
функция `sort_large_user_ids(ids)`, сортирующая список идентификаторов
пользователей методом Шелла, который быстрее простых квадратичных алгоритмов
на средних массивах.
...
Схемы выполнены в редакторе draw.io (исходники `.drawio` прилагаются и
открываются на app.diagrams.net), экспорт в PNG.
